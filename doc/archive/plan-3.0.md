...
- Skill 导出（用户反馈「导出应得 zip 包」）：
  - 原实现 handleExportSkills 只 downloadJSON 元数据清单（name/path/source），不可复用。
  - 依赖安装受阻：pnpm 布局 node_modules 被进程占用（ENOTDIR / ERR_PNPM_REMOVE_MODULES_DIR），
    yazl 装不上 → 自实现 zip-writer.ts：Node zlib.deflateRawSync + 手写 CRC32 + 标准
    Local File Header / Central Directory / EOCD（UTF-8 文件名 bit 11），零依赖跨平台。
  - skills-export.ts：getAllInstalledSkills().byClient 定位 name→目录（含自定义客户端），
    递归收集全部文件打包为 {skillName}/ 结构；单/多 skill 文件名规则；缺目录报错。
  - IPC skills:export-zip → preload → renderer Blob 下载（application/zip）。
  - renderer 类型将 data 声明为 Uint8Array（避免 main 的 Node Buffer 污染 renderer 类型图）。
  - 验证：zip-writer 单测（结构/回读/UTF-8/系统 tar 互操作）+ skills-export 端到端（4+4 用例）。

- Skill 附属文件完整支持（用户确认：编辑后保存一并写入；.source.json 跳过）：
  - importFromZipBuffer 现在返回 files（解包全部附属文件；排除 SKILL.md 与 .source.json；
    路径规范化拒绝 .. 段/绝对路径；解压总量 100MB / 2000 条目上限防 zip 炸弹）。
  - createCustomSkill 增加 files 通道：附件路径**全量预检**（非法即整个拒绝，杜绝
    「SKILL.md 先写、附件失败」的半成品残留——测试首跑即发现并修复）+ 落盘时 assertWithin 双保险；
    逐客户端写入，SKILL.md 用编辑后内容、附件用 zip 原样；不写 .source.json（本地导入=自定义语义）。
  - 链路：CreateSkillModal 保存解析出的 files（编辑模式忽略）→ onSubmit 携带 → Library
    handleCreateSkillSubmit → saveWithCloudSync（input.files 透传）→ createCustomSkill 落盘；
    云端暂存区同步复用 syncSkillToClients（copyDir 整目录复制）天然带上附件，无需改动。
  - 校验：zip 含附件解析返回（3 用例：完整返回/跳过 source.json/穿越拒绝）+ 落盘完整性与
    无半成品断言（3 用例），233/233 全绿。
