...
/**
 * 从 SKILL.md 文本中解析 YAML frontmatter（不引入 yaml 依赖）。
 * 需同时支持两种写法（SkillHub 仓库两者混用）：
 *   1) 行内：`description: Create diagrams...`
 *   2) 折叠块标量：`description: >` / `|`，正文在随后的缩进行中
 *      —— 若不处理，description 会被解析成字面量 ">"，列表将显示空描述。
 */
...
function mapSkillhubEntry(e: SkillhubApiEntry): PlatformSkillListItem | null {
  const slug = e.slug;
  if (!slug) return null;
  const desc = (e.description_zh || e.description || '').trim();
...
function mapClawhubEntry(entry: unknown): PlatformSkillListItem | null {
  ...
  // 真实可下载地址优先：install.sourceUrl / links.source（skills.sh 等外部源直链）
  // > install.reference 为直接的 GitHub owner/repo（如 tuobadaidai/consult-report）→ GitHub 直链
  // > 站内 canonical / SPA 页（由详情页 resolvePlatformSkillUrl 抓取 HTML 兜底）
  let sourceUrl = installSourceUrl || linkSource;
  if (!sourceUrl && ref && !ref.includes(':')) {
    // install.reference 形如 "tuobadaidai/consult-report" 即 GitHub owner/repo，可直接走 GitHub 下载通道
...
export async function resolveDirectSkill(
  ...
  // ModelScope zip 下载直链（/skills/<owner>/<slug>/archive/zip/master）→ 走 zip 解析通道
  if (/modelscope\.cn\/skills\/.+\/archive\/zip/i.test(sourceUrl)) {
    return resolveModelScopeZip(skillsManager, sourceUrl, platform as SupportedPlatform);
  }
...
/**
 * 从 ModelScope 的 zip 下载直链解析 Skill：
 * 下载 zip → 解压到临时目录 → 读取其中的 SKILL.md 作为安装内容。
 * 该通道用于无 GitHub 源的 skill（source_url 为空或指向 modelscope.cn/studios）。
 */
async function resolveModelScopeZip(
  ...
  try {
    ...
    const skill: DiscoveredSkill = {
      ...
      skillMdContent,
      files: [],
      repository: { url: zipUrl, branch: 'master', owner: 'modelscope', repo: skillName },
      downloadUrl: zipUrl,
    };
