...
export default function PlatformServerDetail({ connId, serverId }: Props) {
  ...
  // 默认选中已安装客户端
  useEffect(() => {
    const installed = clients.filter(c => c.installed).map(c => c.id);
  ...
  const envProps = (detail?.envSchema?.properties ?? {}) as Record<string, { description?: string; [k: string]: unknown }>;
  ...
  return (
    <div className="flex flex-col h-full bg-[#1c1c1e]">
      ...
      <div className={`flex items-center gap-2 px-4 h-10 drag-region border-b border-[#3a3a3c] bg-[#1c1c1e]/80 backdrop-blur-xl text-[12px] text-[#636366] ${isMac ? 'pl-20' : ''}`}>
        ...
        <span>/</span>
        <span className="hover:text-white cursor-pointer" onClick={() => navigate('/store')}>{t('store.mcpPlatform') || '平台源'}</span>
        <span>/</span>
              ...
              <div className="flex items-start gap-4 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {detail.isVerified && (
                      <span className="flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-[#0a84ff]/10 text-[#0a84ff] border border-[#0a84ff]/20">
                        <VerifiedIcon className="w-3 h-3" />
                        {t('detail.verified') || 'Verified'}
                      </span>
                    )}
                    {detail.isHosted && (
                      <span className="px-1.5 py-0.5 rounded text-[10px] font-medium bg-[#30d158]/15 text-[#30d158] border border-[#30d158]/20">
        ...
        <div className="w-[280px] flex-shrink-0 border-l border-[#3a3a3c] overflow-y-auto p-4">
          ...
          <div className="card p-4 space-y-3 mb-4">
            ...
            {detail.sourceUrl && (
              <a
                href="#"
                onClick={(e) => { e.preventDefault(); api.system.openExternal(detail.sourceUrl!); }}
            ...
            <a
              href="#"
              onClick={(e) => { e.preventDefault(); api.system.openExternal(`https://modelscope.cn/mcp?name=${encodeURIComponent(serverId)}`); }}
              className="flex items-center justify-between text-[12px] text-[#98989d] hover:text-[#0a84ff] transition-colors"
            >
              <span className="flex items-center gap-2">
            ...
            <div className="space-y-2 text-[12px]">
              ...
              {detail.install && (
                <div className="flex justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    <span className={`w-2 h-2 rounded-full ${runtimeAvailable || runtime === 'docker' ? 'bg-[#34c759]' : 'bg-[#ff9f0a]'}`} />
              ...
              {detail.tags.length > 0 && (
                <div className="flex justify-between">
                  ...
                  <span className="text-white text-right max-w-[160px] truncate">{detail.tags.join(', ')}</span>
                </div>
              )}
            </div>
          </div>
      ...
      <Modal
        ...
        onClose={() => { setShowInstallModal(false); setInstallError(null); }}
      ...
      >
        <div className="space-y-4">
          ...
          {hasEnvForm && (
            <div>
              <label className="block text-[12px] font-medium text-white mb-2">{t('detail.envVars') || 'Environment Variables'}</label>
                  ...
                  return (
                    <div key={key}>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {schema?.description && (
                          <span className="text-[10px] text-[#636366] truncate">{(schema.description as string)}</span>
                        )}
                      </div>
                      <input
                        type="text"
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => { setShowInstallModal(false); setInstallError(null); }} className="btn btn-secondary">{t('common.cancel')}</button>
            <button
