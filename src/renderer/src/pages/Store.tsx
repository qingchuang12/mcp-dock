...
export default function Store() {
  ...
  const { data: mcpPlatformData, isLoading: isLoadingPlatform, error: platformError, refetch: refetchPlatform } = useQuery({
  ...
  const { data: skillsDirectFirst, isLoading: isLoadingSkillsDirect, error: skillsDirectError, refetch: refetchSkillsDirect } = useQuery({
  ...
  useEffect(() => {
    api.apiConnections.list('skill').then(setConnections).catch(() => setConnections([]));
  }, [api]);
  ...
  // 下拉当前值：用户已选连接优先；否则优先用「设为默认」的源（kind=mcp），
  // 再回退到 dataSource 对应内置源，最后回退列表首条
  const selectedMcpSourceId =
    mcpConnId ??
    mcpSources.find(c => c.isDefault && (c.kind ?? 'mcp') === 'mcp')?.id ??
  ...
  return (
    <div className="flex flex-col h-full bg-[#1c1c1e]">
      ...
      {!isLoading && !error && result.items.length > 0 && (
        <div className="flex items-center justify-between gap-3 px-4 py-2 border-t border-[#3a3a3c] bg-[#2c2c2e]">
