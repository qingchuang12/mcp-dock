    ...
    <div className="p-4">
      <div className="flex items-center gap-2 mb-3">
        <span className="text-[12px] text-[#98989d]">
          ...
          <span className="ml-1 px-1.5 py-0.5 rounded bg-[#0a84ff]/15 text-[#0a84ff] text-[10px]">{PLATFORM_META[platformType]?.label ?? platformType}</span>
        </span>
        {query && (
      ...
      {loading ? (
        ...
        <div className="py-8 px-4 text-center">
          <div className={`w-10 h-10 mx-auto mb-3 rounded-full flex items-center justify-center ${unsupported ? 'bg-[#0a84ff]/15' : 'bg-[#ff9f0a]/15'}`}>
            <span className={`text-[18px] leading-none ${unsupported ? 'text-[#0a84ff]' : 'text-[#ff9f0a]'}`}>!</span>
          </div>
          ...
          {diag && diag.attempts.length > 0 && (
            <>
              ...
              {showDiag && (
                <div className="mt-3 text-left rounded-md bg-[#1c1c1e] border border-[#3a3a3c] overflow-hidden">
                  <div className="px-3 py-2 border-b border-[#3a3a3c] text-[10px] text-[#636366]">
                    平台 {diag.platform} · Base {diag.baseUrl} · 令牌 {diag.authorized ? '已附带' : '未附带'}
                  </div>
                  {diag.attempts.map((a, i) => (
                    <div key={i} className="px-3 py-2 border-b border-[#3a3a3c] last:border-0">
                      <div className="flex items-start gap-2">
                        <span className={a.ok ? 'text-[#34c759]' : 'text-[#ff3b30]'}>{a.ok ? '✓' : '✗'}</span>
                        <div className="min-w-0 flex-1">
                          ...
                          {a.message && (
                            <div className="text-[10px] text-[#ff9f0a] mt-0.5">{a.message}</div>
                          )}
                        </div>
                      </div>
                    </div>
          ...
          {items.map(it => (
            <div key={it.id} className="p-3 rounded-md bg-[#3a3a3c]/40">
              ...
              <div className="flex items-center justify-between mt-3">
                <span className="text-[10px] text-[#636366] truncate max-w-[60%]">{it.sourceUrl}</span>
                <button
