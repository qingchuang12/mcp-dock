...
export default function TokenManager() {
  ...
  const resetForm = () => {
    setName(''); setPlatform('modelscope'); setRawKey(''); setExpiry(null);
  };

  const handleImport = async () => {
    if (!name.trim()) { toast.error('请输入令牌名称'); return; }
    if (!rawKey.trim()) { toast.error('请粘贴平台 API key'); return; }
    try {
  ...
  const handleRevoke = async (id: string, revoked: boolean) => {
    if (revoked) { await api.apiTokens.restore(id); toast.success('已恢复令牌'); }
    else { await api.apiTokens.revoke(id); toast.success('已撤销令牌'); }
    await load();
  };
  ...
  return (
    <div className="card p-4">
      ...
      <p className="text-[12px] text-[#98989d] mb-3">
        粘贴来自 ModelScope / SafeSkill / SkillHub 等平台的 API key，用于直连平台搜索与下载 skill。令牌仅以加密形式存储。导入时无需包含 <code className="text-[#5ac8fa]">Bearer</code> 前缀，前后空格会自动去除。
      </p>
            ...
            <div key={tk.id} className="p-3 rounded-md bg-[#3a3a3c]/40">
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {tk.revoked && (
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#ff9f0a]/15 text-[#ff9f0a]">已撤销</span>
                    )}
                  </div>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {tk.scopes.map(s => (
                      <span key={s} className={`text-[10px] px-1.5 py-0.5 rounded ${scopeColor(s)}`}>{s}</span>
                    ))}
                  </div>
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  <IconBtn title="复制" onClick={() => handleCopy(revealed[tk.id] || '')}>
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 7.5V6.108c0-1.135.845-2.098 1.976-2.192.373-.03.748-.03 1.125-.03h3.448c.426 0 .801 0 1.175.03 1.13.094 1.976 1.057 1.976 2.192V16.5c0 1.135-.845 2.098-1.976 2.192a12.12 12.12 0 01-1.175.03H8.25m0-11.25v11.25" />
                    </svg>
                  </IconBtn>
                  <IconBtn title={tk.revoked ? '恢复' : '撤销'} onClick={() => handleRevoke(tk.id, tk.revoked)}>
                    <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              {revealed[tk.id] && (
                <div className="mt-2 flex items-center gap-2">
                  <code className="flex-1 px-2 py-1.5 rounded bg-[#1c1c1e] border border-[#3a3a3c] text-[11px] text-[#5ac8fa] font-mono break-all">
                    {revealed[tk.id]}
                  </code>
                  <button onClick={() => handleCopy(revealed[tk.id])} className="text-[11px] text-[#0a84ff] hover:underline">复制</button>
                </div>
              )}
            </div>
          ))}
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="导入平台 API 令牌">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => setShowCreate(false)} className="px-3 py-1.5 rounded-md bg-[#3a3a3c] text-white text-[12px] hover:bg-[#3a3a3c]/80 transition-colors">取消</button>
            <button onClick={handleImport} className="px-3 py-1.5 rounded-md bg-[#0a84ff] text-white text-[12px] font-medium hover:bg-[#0a84ff]/90 transition-colors">导入</button>
          </div>
        </div>
      </Modal>
    </div>
