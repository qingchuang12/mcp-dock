/**
 * API 直连管理卡片
 * 展示所有连接（状态点/名称/平台/详情），支持新增/编辑/删除、搜索筛选、导出配置、一键默认直连。
...
const DEFAULT_PLATFORMS: PlatformType[] = ['modelscope', 'safeskill', 'skillhub', 'skillsmp', 'clawhub'];
...
export default function ConnectionManager() {
  ...
  const load = async () => {
    try {
      const [c, t] = await Promise.all([api.apiConnections.list('skill'), api.apiTokens.list()]);
      setConns(c);
      setTokens(t);
    } catch (e) {
  ...
  const openCreate = () => {
    setEditing({ name: '', platformType: 'modelscope', baseUrl: PLATFORM_META.modelscope.defaultBaseUrl, tokenId: undefined, customHeaders: {}, detail: '', kind: 'skill', enabled: true });
    setShowEdit(true);
  };
  ...
  const handleSave = async () => {
    if (!editing || !editing.name?.trim()) { toast.error('请输入连接名称'); return; }
    try {
  ...
  const handleCreateDefaults = async () => {
    for (const p of DEFAULT_PLATFORMS) {
      const exists = conns.find(c => c.platformType === p);
      if (!exists) {
        await api.apiConnections.create({
          name: `${PLATFORM_META[p].label} 官方直连`,
          platformType: p,
          baseUrl: PLATFORM_META[p].defaultBaseUrl,
          tokenId: undefined,
          customHeaders: {},
          detail: '',
          kind: 'skill',
  ...
  return (
    <div className="card p-4">
      <div className="flex items-center justify-between mb-1">
        <h2 className="text-[13px] font-semibold text-white">Skill 源管理</h2>
      ...
      <p className="text-[12px] text-[#98989d] mb-3">
        连接绑定一个令牌后即可在 Store 页驱动对应平台 skill 的搜索与下载。导出配置不含令牌明文。
      </p>
      ...
      {loading ? (
        ...
        <div className="text-[12px] text-[#636366] py-3 text-center">
          {conns.length === 0 ? '尚未创建任何连接，点击「新建连接」「一键默认直连」或「恢复内置源」' : '无匹配的连接'}
        </div>
      ) : (
        <div className="space-y-2">
          {filtered.map(c => {
            ...
            const isBuiltin = c.id === BUILTIN_SKILL_SOURCE_IDS.github;
            return (
              <div key={c.id} className="p-3 rounded-md bg-[#3a3a3c]/40">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {isBuiltin && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#30d158]/15 text-[#30d158] flex-shrink-0">内置</span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <IconBtn title="验证" onClick={() => handleVerify(c.id)}>
                    ...
                    <IconBtn title={isBuiltin ? '内置来源不可删除' : '删除'} danger disabled={isBuiltin} onClick={() => handleDelete(c.id)}>
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <div className="mt-2 flex items-start gap-1.5">
                  ...
                  <p className="text-[10px] text-[#636366] leading-relaxed">
                    ...
                    {tk
                      ...
                      : <span className="text-[#ff9f0a]"> 未绑定令牌（搜索/下载将不带鉴权）</span>}
                  </p>
                </div>
      ...
      {/* 编辑/新建弹窗 */}
      <Modal isOpen={showEdit} onClose={() => setShowEdit(false)} title={editing?.id ? '编辑连接' : '新建连接'}>
        {editing && (
          <div className="space-y-4">
            ...
            <div>
              ...
              {SKILL_PLATFORM_TYPES.includes(editing.platformType as PlatformType) ? (
                ...
                <div className="w-full px-3 py-2 rounded-md bg-[#1c1c1e] border border-[#3a3a3c] text-white text-[12px] flex items-center justify-between">
                  <span>{PLATFORM_META[editing.platformType as PlatformType]?.label || editing.platformType}</span>
                  <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#30d158]/15 text-[#30d158]">内置来源（不可更改类型）</span>
                </div>
              )}
            </div>
            <div>
