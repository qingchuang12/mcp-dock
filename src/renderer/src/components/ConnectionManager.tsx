    ...
    return (
        <div className="card p-4">
            <div className="flex items-center justify-between mb-1">
                <h2 className="text-[13px] font-semibold text-[var(--color-text)]">{t('skillSource.title')}</h2>
                    ...
                    {filtered.map(c => {
                        ...
                        return (
                          ...
                          {platformLabel(t, c.platformType)}
                                                ...
                                                {c.isDefault && (
                                                    <span
                                                        className="text-[12px] px-1.5 py-0.5 rounded bg-[#ff9f0a]/15 text-[#ff9f0a] flex-shrink-0">{t('skillSource.default')}</span>
                                                )}
                                                {isBuiltin && (
                                                    <span
                                                        className="text-[12px] px-1.5 py-0.5 rounded bg-[#98989d]/15 text-[var(--color-muted2)] flex-shrink-0">{t('skillSource.builtin')}</span>
                                                )}
                                                {!enabled && (
                                                    <span
                                                        className="text-[12px] px-1.5 py-0.5 rounded bg-[#ff9f0a]/15 text-[#ff9f0a] flex-shrink-0">{t('skillSource.disabledBadge')}</span>
                                                )}
                                            </div>
                      ...
                      />
                                ...
                                <div className="mt-2 flex items-start gap-1.5">
                                    ...
                                    <p className="text-[12px] text-[var(--color-muted)] leading-relaxed">
                                        ...
                                        {tk
                                            ? <span
                                                className="text-[var(--color-muted2)]"> {tk.name}（{t('skillSource.scope')} {tk.scopes.join(', ')} · {t('skillSource.expiry')} {tk.expiresAt ? new Date(tk.expiresAt).toLocaleDateString() : t('skillSource.noExpiry')} · {tk.revoked ? t('skillSource.revoked') : t('skillSource.normal')}）</span>
                                            : <span className="text-[#ff9f0a]"> {t('skillSource.noToken')}</span>}
                                    </p>
                                </div>
                                {c.detail &&
                                    <p className="text-[12px] text-[var(--color-muted)] mt-1 truncate">{t('skillSource.detailPrefix')}{c.detail}</p>}
                            </div>
                        );
                    })}
                </div>
            ...
            <Modal isOpen={showEdit} onClose={() => setShowEdit(false)}
                ...
                {editing && (
                    <div className="space-y-4">
                        <div>
                            <label className="block text-[12px] text-[var(--color-muted2)] mb-1.5">{t('skillSource.name')}</label>
                            <input
                                value={editing.name || ''}
                        ...
                        <div>
                            <label
                                className="block text-[12px] text-[var(--color-muted2)] mb-1.5">{t('skillSource.platformType')}</label>
                        ...
                        <div>
                            <label
                                className="block text-[12px] text-[var(--color-muted2)] mb-1.5">{t('skillSource.bindToken')}</label>
                            <select
                        ...
                        <div>
                            <label
                                className="block text-[12px] text-[var(--color-muted2)] mb-1.5">{t('skillSource.detailOptional')}</label>
                            <input
                        ...
                        <div className="flex items-center justify-between">
                            <label className="text-[12px] text-[var(--color-muted2)]">{t('skillSource.enableSource')}</label>
                            <button
